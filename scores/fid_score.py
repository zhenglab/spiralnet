...
def _compute_statistics_of_path(path, model, batch_size, dims, cuda):
    ...
    if os.path.exists(npz_file):
        ...
        files = list(path.glob('*.jpg')) + list(path.glob('*.png')) + list(path.glob('*.JPG'))
        ...
        count = 0
        for fn in  files:
            img  = imread(str(fn)).astype(np.float32)
            count += 1
            
            print(fn, " ",count, " img shape: ",img.shape)
            
            if len(img.shape) != 3:
                print("img shape: ",img.shape)
                R = img
                G = img
                B = (img - 0.299 * R - 0.587 * G) / 0.114
                grayRGB = np.zeros((img.shape[0],img.shape[1],3))
                grayRGB[:,:,0] = R
                grayRGB[:,:,1] = G
                grayRGB[:,:,2] = B
                img = grayRGB
                print("grayRGB shape: ",grayRGB.shape)
        ...
        # print("img shape: ", imgs)
        print("img shape: ", imgs.shape)
        imgs = imgs.transpose((0, 3, 1, 2))  
        # img shape:  (915, 256, 256, 3)
        # img shape:  (1189, 256, 256, 3)
